...
void idt_init(void) {
    ...
    idtp.limit = sizeof(idt) - 1; idtp.base = (uint32_t)&idt;
...
static void page_fault_dump(uint32_t *frame, uint32_t fault_address) {
    ...
    console_write("  address: 0x"); console_write_hex(fault_address);
    console_write("  error: 0x"); console_write_hex(frame[13]); console_write("\n");
}
...
uint32_t *interrupt_dispatch(uint32_t *frame) {
    ...
    if (vector == 128) {
        ...
        if (number == SYS_FORK) {
            frame[11] = process_fork_current(frame) ? process_current()->pid + 1u : SYSCALL_ERR;
            if (frame[11] == SYSCALL_ERR) return frame;
            return frame;
        }
        ...
        if (number == SYS_YIELD || number == SYS_EXIT || number == SYS_EXEC) {
            ...
            if (cur) tss_set_kernel_stack(process_kernel_stack_top(cur));
            return next ? next : frame;
        }
        return frame;
    }
    if (vector == 32) {
        ++ticks; outb(0x20, 0x20);
    ...
    if (vector == 33) { keyboard_handle_scancode(inb(0x60)); outb(0x20, 0x20); }
        ...
        if ((frame[15] & 0x3u) == 0x3u) {
            page_fault_dump(frame, fault_address); process_exit_current();
            uint32_t *next = process_schedule(frame); struct process *cur = process_current();
            if (cur) tss_set_kernel_stack(process_kernel_stack_top(cur)); return next ? next : frame;
        }
        console_write("\n[FATAL] kernel page fault at 0x"); console_write_hex(fault_address);
        console_write(" error=0x"); console_write_hex(frame[13]); console_write("\nSystem halted.\n");
