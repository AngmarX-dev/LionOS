...
uint32_t ramfs_count(void){return count+5u;}
const char*ramfs_name(uint32_t i){if(i<count)return files[i].name;if(i==count)return "hello.elf";if(i==count+1u)return "process_test.elf";if(i==count+2u)return "ipc_test.elf";if(i==count+3u)return "signal_test.elf";if(i==count+4u)return "net_test.elf";return 0;}
const char*ramfs_data(const char*n){if(streq(n,"hello.elf"))return(const char*)_binary_hello_elf_start;if(streq(n,"process_test.elf"))return(const char*)_binary_process_test_elf_start;if(streq(n,"ipc_test.elf"))return(const char*)_binary_ipc_test_elf_start;if(streq(n,"signal_test.elf"))return(const char*)_binary_signal_test_elf_start;if(streq(n,"net_test.elf"))return(const char*)_binary_net_test_elf_start;struct ramfs_file*f=find(n);return f?f->data:0;}
uint32_t ramfs_size(const char*n){if(streq(n,"hello.elf"))return(uint32_t)(_binary_hello_elf_end-_binary_hello_elf_start);if(streq(n,"process_test.elf"))return(uint32_t)(_binary_process_test_elf_end-_binary_process_test_elf_start);if(streq(n,"ipc_test.elf"))return(uint32_t)(_binary_ipc_test_elf_end-_binary_ipc_test_elf_start);if(streq(n,"signal_test.elf"))return(uint32_t)(_binary_signal_test_elf_end-_binary_signal_test_elf_start);if(streq(n,"net_test.elf"))return(uint32_t)(_binary_net_test_elf_end-_binary_net_test_elf_start);struct ramfs_file*f=find(n);return f?f->size:0;}
int ramfs_exists(const char*n){return streq(n,"hello.elf")||streq(n,"process_test.elf")||streq(n,"ipc_test.elf")||streq(n,"signal_test.elf")||streq(n,"net_test.elf")||find(n)!=0;}
int ramfs_write(const char*n,const char*d,uint32_t s){if(!n||!*n||!d||streq(n,"hello.elf")||streq(n,"process_test.elf")||streq(n,"ipc_test.elf")||streq(n,"signal_test.elf")||streq(n,"net_test.elf"))return-1;struct ramfs_file*f=find(n);if(!f){if(count>=RAMFS_MAX_FILES-5u)return-1;f=&files[count++];copy_name(f->name,n);}if(s>RAMFS_DATA_MAX)s=RAMFS_DATA_MAX;for(uint32_t i=0;i<s;++i)f->data[i]=d[i];f->size=s;return 0;}
int ramfs_remove(const char*n){if(streq(n,"hello.elf")||streq(n,"process_test.elf")||streq(n,"ipc_test.elf")||streq(n,"signal_test.elf")||streq(n,"net_test.elf"))return-1;for(uint32_t i=0;i<count;++i)if(streq(files[i].name,n)){for(uint32_t j=i+1u;j<count;++j)files[j-1u]=files[j];--count;return 0;}return-1;}
