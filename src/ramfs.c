...
int ramfs_exists(const char *name) { return streq(name, "hello.elf") || find(name) != 0; }

int ramfs_write(const char *name, const char *data, uint32_t size) {
    if (!name || !*name || !data || streq(name, "hello.elf")) return -1;
    struct ramfs_file *f = find(name);
    if (!f) { if (count >= RAMFS_MAX_FILES - 1u) return -1; f = &files[count++]; copy_name(f->name, name); }
...
int ramfs_remove(const char *name) {
    if (streq(name, "hello.elf")) return -1;
