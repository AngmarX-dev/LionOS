...
static int write_user_u32(const struct process *p,uint32_t ptr,uint32_t value) {
    ...
    if(!p||!ptr||ptr>0xBFFFFFFCu||(ptr&(sizeof(uint32_t)-1u)))return -1;
    if(paging_get_user_page(p->page_directory,ptr&~(PAGE_SIZE-1u),&physical,&flags)!=0)return -1;
    (void)flags;*(uint32_t *)(uintptr_t)(physical+(ptr&(PAGE_SIZE-1u)))=value;return 0;
}
...
void process_init(void){for(uint32_t i=0;i<LIONOS_PROCESS_MAX;++i)clear_process(&processes[i]);next_pid=2;processes[0].pid=1;processes[0].state=PROCESS_RUNNING;current=&processes[0];init_bootstrap_frame();}
...
int process_exec_replace_current(uint32_t entry,uint32_t stack,uint32_t pd,const uint32_t *pages,const uint32_t *vas,uint32_t count){
    if(!current||current==&processes[0]||!pd||!pages||!vas||!count||count>LIONOS_PROCESS_MAX_USER_PAGES)return -1;
...
int32_t process_waitpid(uint32_t pid,uint32_t status_ptr){
    ...
    if(z){if(write_user_u32(current,status_ptr,z->exit_code)!=0)return -1;int32_t result=(int32_t)z->pid;reap_process(z);return result;}
...
static void wake_waiting_parent(struct process *child){for(uint32_t i=1;i<LIONOS_PROCESS_MAX;++i){struct process *p=&processes[i];if(p->state!=PROCESS_WAITING||p->pid!=child->parent_pid)continue;if(p->wait_pid!=PROCESS_WAIT_ANY&&p->wait_pid!=child->pid)continue;uint32_t *f=process_saved_frame(p);if(f)f[11]=child->pid;if(p->wait_status_ptr)write_user_u32(p,p->wait_status_ptr,child->exit_code);p->wait_pid=0;p->wait_status_ptr=0;p->state=PROCESS_READY;child->reap_pending=1;return;}}
...
uint32_t *process_schedule(uint32_t *frame){
    if(!current)return frame;struct process *prev=current;if(current!=&processes[0])current->saved_frame=(uint32_t)(uintptr_t)frame;
    if(current->deferred_kernel_stack){page_free((void *)(uintptr_t)(current->deferred_kernel_stack-4096u));current->deferred_kernel_stack=0;}
    uint32_t start=process_index(current);for(uint32_t step=1;step<=LIONOS_PROCESS_MAX;++step){uint32_t idx=(start+step)%LIONOS_PROCESS_MAX;struct process *c=&processes[idx];if(c->state!=PROCESS_READY)continue;if(current->state==PROCESS_RUNNING)current->state=PROCESS_READY;current=c;current->state=PROCESS_RUNNING;paging_switch_address_space(current->page_directory);tss_set_kernel_stack(current->kernel_stack_top);if(prev->reap_pending)reap_process(prev);return process_saved_frame(current);}
    if(current->state==PROCESS_ZOMBIE){current=&processes[0];current->state=PROCESS_RUNNING;paging_switch_address_space(current->page_directory);tss_set_kernel_stack(current->kernel_stack_top);if(prev->reap_pending)reap_process(prev);return process_saved_frame(current);}current->state=PROCESS_RUNNING;tss_set_kernel_stack(current->kernel_stack_top);return frame;
}
