...
static void clear_process(struct process *p) {
    p->pid=0; p->state=PROCESS_UNUSED; p->parent_pid=0; p->exit_code=0; p->wait_pid=0; p->reap_pending=0;
...
void process_init(void){
    ...
    next_pid=2;processes[0].pid=1;processes[0].state=PROCESS_RUNNING;processes[0].parent_pid=0;current=&processes[0];init_bootstrap_frame();
}
...
const char *process_state_name(uint32_t s){switch(s){case PROCESS_READY:return "READY";case PROCESS_RUNNING:return "RUNNING";case PROCESS_ZOMBIE:return "ZOMBIE";case PROCESS_WAITING:return "WAITING";default:return "UNUSED";}}
...
static int is_child_of(const struct process *child, uint32_t parent_pid, uint32_t pid) {
    return child->state!=PROCESS_UNUSED && child->parent_pid==parent_pid && (pid==PROCESS_WAIT_ANY || child->pid==pid);
}
static struct process *find_zombie_child(uint32_t parent_pid, uint32_t pid) {
    for(uint32_t i=1;i<LIONOS_PROCESS_MAX;++i) if(is_child_of(&processes[i],parent_pid,pid)&&processes[i].state==PROCESS_ZOMBIE) return &processes[i];
    return 0;
}
static int has_child(uint32_t parent_pid, uint32_t pid) {
    for(uint32_t i=1;i<LIONOS_PROCESS_MAX;++i) if(is_child_of(&processes[i],parent_pid,pid)) return 1;
    return 0;
}
...
int32_t process_waitpid(uint32_t pid) {
    if(!current||current==&processes[0]) return -1;
    struct process *z=find_zombie_child(current->pid,pid);
    if(z){int32_t result=(int32_t)z->pid;reap_process(z);return result;}
    if(!has_child(current->pid,pid)) return -1;
    current->wait_pid=pid;current->state=PROCESS_WAITING;return PROCESS_WAIT_BLOCKED;
}

static void wake_waiting_parent(struct process *child) {
    for(uint32_t i=1;i<LIONOS_PROCESS_MAX;++i){struct process *p=&processes[i];if(p->state!=PROCESS_WAITING||p->pid!=child->parent_pid)continue;if(p->wait_pid!=PROCESS_WAIT_ANY&&p->wait_pid!=child->pid)continue;uint32_t *f=process_saved_frame(p);if(f)f[11]=child->pid;p->wait_pid=0;p->state=PROCESS_READY;child->reap_pending=1;return;}
}
...
void process_exit_current(uint32_t exit_code){if(!current||current==&processes[0])return;current->exit_code=exit_code;current->state=PROCESS_ZOMBIE;wake_waiting_parent(current);}
...
uint32_t *process_schedule(uint32_t *frame){
    if(!current)return frame;struct process *prev=current;
    if(current!=&processes[0]) current->saved_frame=(uint32_t)(uintptr_t)frame;
    uint32_t start=process_index(current);
    for(uint32_t step=1;step<=LIONOS_PROCESS_MAX;++step){uint32_t idx=(start+step)%LIONOS_PROCESS_MAX;struct process *c=&processes[idx];if(c->state!=PROCESS_READY)continue;if(current->state==PROCESS_RUNNING)current->state=PROCESS_READY;current=c;current->state=PROCESS_RUNNING;paging_switch_address_space(current->page_directory);tss_set_kernel_stack(current->kernel_stack_top);if(prev->reap_pending)reap_process(prev);return process_saved_frame(current);}
    if(current->state==PROCESS_ZOMBIE){current=&processes[0];current->state=PROCESS_RUNNING;paging_switch_address_space(current->page_directory);tss_set_kernel_stack(current->kernel_stack_top);if(prev->reap_pending)reap_process(prev);return process_saved_frame(current);}
