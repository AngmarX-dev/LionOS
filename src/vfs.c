...
static int copy_path(char *dst,const char *src){if(!dst||!src||!*src)return-1;uint32_t i=0;while(src[i]&&i<VFS_PATH_MAX-1u){if(src[i]=='/')return-1;dst[i]=src[i];++i;}if(src[i])return-1;dst[i]=0;return 0;}
static uint32_t backend_for(const char *p){if(ramfs_data(p))return VFS_BACKEND_RAMFS;if(diskfs_available()&&diskfs_exists(p))return VFS_BACKEND_DISKFS;return VFS_BACKEND_NONE;}
...
int vfs_remove(const char*path){if(ramfs_data(path))return ramfs_remove(path);if(diskfs_available()&&diskfs_exists(path))return diskfs_remove(path);return-1;}
int vfs_stat_path(const char*path,struct vfs_stat*st){if(!path||!st)return-1;if(ramfs_data(path)){st->size=ramfs_size(path);st->backend=VFS_BACKEND_RAMFS;st->flags=0;return 0;}if(diskfs_available()&&diskfs_exists(path)){st->size=diskfs_size(path);st->backend=VFS_BACKEND_DISKFS;st->flags=0;return 0;}return-1;}
const char*vfs_name(uint32_t index){uint32_t n=ramfs_count();if(index<n)return ramfs_name(index);return diskfs_name(index-n);}
uint32_t vfs_count(void){return ramfs_count()+(diskfs_available()?diskfs_count():0u);}
