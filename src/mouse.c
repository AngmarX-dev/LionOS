...
static int mouse_command(uint8_t command) {
    /*
     * The PS/2 controller has one shared output byte.  A mouse command can
     * leave an additional BAT/status byte (0xAA) behind the ACK.  If that
     * byte is left in the controller, keyboard_poll() sees AUX=1 and refuses
     * to consume keyboard data.  That makes both keyboard and mouse appear
     * dead on real hardware.
     */
    flush_output();
    ...
    /* Discard BAT/status bytes generated by the command before streaming. */
    flush_output();
    return 0;
}
...
int mouse_init(void) {
    ...
    /*
     * Some physical PS/2 controllers/devices report BAT/status bytes after
     * reset.  Drain them before issuing the first mouse command so the ACK
     * handshake cannot consume the wrong byte.
     */
