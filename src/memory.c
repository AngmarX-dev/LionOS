...
void*page_alloc_contiguous(uint32_t count){if(!count||count>MAX_PAGES-RESERVED_PAGES)return 0;uint32_t irq;spinlock_acquire_irqsave(&memory_lock,&irq);uint32_t run=0,start=RESERVED_PAGES;for(uint32_t p=RESERVED_PAGES;p<MAX_PAGES;++p){if(is_free(p)){if(!run)start=p;if(++run==count){for(uint32_t i=0;i<count;++i)mark_used(start+i);free_pages-=count;spinlock_release_irqrestore(&memory_lock,irq);return(void*)(uintptr_t)(start*PAGE_SIZE);}}else run=0;}spinlock_release_irqrestore(&memory_lock,irq);return 0;}
void*page_alloc(void){return page_alloc_contiguous(1);}
void page_free(void*ptr){uint32_t a=(uint32_t)(uintptr_t)ptr;if((a&(PAGE_SIZE-1u))!=0)return;uint32_t p=a/PAGE_SIZE;if(p<RESERVED_PAGES||p>=MAX_PAGES)return;uint32_t irq;spinlock_acquire_irqsave(&memory_lock,&irq);if(!is_free(p)){mark_free(p);++free_pages;}spinlock_release_irqrestore(&memory_lock,irq);}
uint32_t memory_total_pages(void){return total_pages;}uint32_t memory_free_pages(void){uint32_t irq,n;spinlock_acquire_irqsave(&memory_lock,&irq);n=free_pages;spinlock_release_irqrestore(&memory_lock,irq);return n;}
