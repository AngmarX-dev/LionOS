...
static void command(char *cmd) {
    if (eq(cmd, "help")) {
        console_write("LionOS Shell v0.4\n");
        console_write("help clear echo mem ps ls cat write touch rm uname uptime version about\n");
        console_write("cat <file> | write <file> <text> | touch <file> | rm <file>\n");
        ...
        for (uint32_t i = 0; i < ramfs_count(); ++i) {
            console_write(ramfs_name(i)); console_putc('\n');
        }
        ...
        char *name = skip_spaces(cmd + 6);
        if (!*name || ramfs_write(name, "", 0) < 0) console_write("touch: failed\n");
        else console_write("touch: ok\n");
    } else if (prefix(cmd, "rm ")) {
        char *name = skip_spaces(cmd + 3);
        if (!*name || ramfs_remove(name) < 0) console_write("rm: file not found\n");
        else console_write("rm: ok\n");
    } else if (eq(cmd, "uname")) {
        console_write("LionOS 0.4 x86 i386 kernel\n");
    } else if (eq(cmd, "version")) {
        console_write("LionOS version 0.4\n");
    } else if (eq(cmd, "about")) {
        console_write("LionOS: experimental 32-bit x86 operating system.\n");
        console_write("Kernel, paging, ring-3 processes, scheduler, syscalls and RAMFS.\n");
    } else if (eq(cmd, "uptime")) {
        console_write("Uptime: scheduler running at 100 Hz.\n");
    } else if (eq(cmd, "run")) {
        console_write("run: userspace test programs are managed by the process scheduler.\n");
    } else if (*cmd) {
...
void shell_run(void) {
    ...
    console_write("          LionOS Shell v0.4\n");
    console_write("========================================\n");
    console_write("RAMFS + process diagnostics enabled.\n");
    ...
    for (;;) {
        ...
        } else if (c >= 32 && c < 127 && len < SHELL_LINE_MAX - 1u) {
            line[len++] = (char)c;
            console_putc((char)c);
        }
    }
