...
static void write_directory_sector(uint32_t s){zero(sector,ATA_SECTOR_SIZE);const uint8_t *src=(const uint8_t*)entries+s*ATA_SECTOR_SIZE;for(uint32_t i=0;i<ATA_SECTOR_SIZE;++i)sector[i]=src[i];}
static int persist_directory(uint32_t index){return ata_write_sector(DISKFS_DIR_LBA+index/16u,(uint8_t*)entries+index/16u*ATA_SECTOR_SIZE);}
static int write_directory_sector_safe(uint32_t s){zero(sector,ATA_SECTOR_SIZE);const uint8_t *src=(const uint8_t*)entries+s*ATA_SECTOR_SIZE;for(uint32_t i=0;i<ATA_SECTOR_SIZE;++i)sector[i]=src[i];return ata_write_sector(DISKFS_DIR_LBA+s,sector);}
...
static int format(void){zero(entries,sizeof(entries));file_count=0;if(save_super()<0)return-1;for(uint32_t i=0;i<DISKFS_DIR_SECTORS;++i)if(write_directory_sector_safe(i)<0)return-1;return 0;}
...
const char *diskfs_name(uint32_t index){if(!available)return 0;uint32_t seen=0;for(uint32_t i=0;i<DISKFS_MAX_FILES;++i){if(!entries[i].name[0])continue;if(seen==index)return entries[i].name;++seen;}return 0;}
