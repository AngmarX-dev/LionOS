...
#define DISKFS_DIR_SECTORS 5u
#define DISKFS_DATA_LBA 6u
...
static int write_directory_sector(uint32_t s){zero(sector,ATA_SECTOR_SIZE);uint32_t off=s*ATA_SECTOR_SIZE;if(off>=sizeof(entries))return-1;uint32_t n=sizeof(entries)-off;if(n>ATA_SECTOR_SIZE)n=ATA_SECTOR_SIZE;const uint8_t *src=((const uint8_t*)entries)+off;for(uint32_t i=0;i<n;++i)sector[i]=src[i];return ata_write_sector(DISKFS_DIR_LBA+s,sector);}
...
int diskfs_init(void){available=0;file_count=0;zero(entries,sizeof(entries));if(ata_init()<0)return-1;if(ata_read_sector(DISKFS_SUPER_LBA,sector)<0)return-1;if(sector[0]!='L'||sector[1]!='I'||sector[2]!='O'||sector[3]!='N'||sector[4]!='F'||sector[5]!='S'||sector[6]!='1'||*(uint32_t*)&sector[8]!=DISKFS_VERSION){if(format()<0)return-1;}else{file_count=*(uint32_t*)&sector[12];if(file_count>DISKFS_MAX_FILES)return-1;for(uint32_t i=0;i<DISKFS_DIR_SECTORS;++i)if(ata_read_sector(DISKFS_DIR_LBA+i,sector)<0)return-1;else{uint32_t off=i*ATA_SECTOR_SIZE;uint32_t n=sizeof(entries)-off;if(n>ATA_SECTOR_SIZE)n=ATA_SECTOR_SIZE;uint8_t *dst=((uint8_t*)entries)+off;for(uint32_t j=0;j<n;++j)dst[j]=sector[j];}}uint32_t actual=0;for(uint32_t i=0;i<DISKFS_MAX_FILES;++i){if(!entries[i].name[0])continue;if(entries[i].size>DISKFS_MAX_FILE_SIZE||entries[i].start_lba!=DISKFS_DATA_LBA+i*DISKFS_SECTORS_PER_FILE)entries[i].name[0]=0;else ++actual;}file_count=actual;available=1;return 0;}
