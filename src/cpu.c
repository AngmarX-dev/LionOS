...
static uint32_t acpi_find_madt(uint32_t rsdt,uint64_t xsdt,uint8_t revision){
    if(revision>=2u && (uint32_t)(xsdt>>32)==0u && xsdt){
        const struct acpi_sdt*h=(const struct acpi_sdt*)(uintptr_t)(uint32_t)xsdt;
        if(sig4(h->sig,"XSDT")&&h->length>=36u&&checksum8((const uint8_t*)h,h->length)==0u){
            uint32_t count=(h->length-36u)/8u;const uint8_t*p=(const uint8_t*)h+36u;
            for(uint32_t i=0;i<count;++i){uint64_t a=*(const uint64_t*)(const void*)(p+i*8u);if((uint32_t)(a>>32))continue;const struct acpi_sdt*t=(const struct acpi_sdt*)(uintptr_t)(uint32_t)a;if(t->length>=36u&&sig4(t->sig,"APIC")&&checksum8((const uint8_t*)t,t->length)==0u)return(uint32_t)a;}
        }
    }
    if(rsdt){
        const struct acpi_sdt*h=(const struct acpi_sdt*)(uintptr_t)rsdt;
        if(sig4(h->sig,"RSDT")&&h->length>=36u&&checksum8((const uint8_t*)h,h->length)==0u){
            uint32_t count=(h->length-36u)/4u;const uint8_t*p=(const uint8_t*)h+36u;
            for(uint32_t i=0;i<count;++i){uint32_t a=*(const uint32_t*)(const void*)(p+i*4u);const struct acpi_sdt*t=(const struct acpi_sdt*)(uintptr_t)a;if(t->length>=36u&&sig4(t->sig,"APIC")&&checksum8((const uint8_t*)t,t->length)==0u)return a;}
        }
    }
    return 0;
}
...
void cpu_init(uint32_t multiboot_info){
    reset_cpus();
    uint32_t a,b,c,d;__asm__ volatile("cpuid":"=a"(a),"=b"(b),"=c"(c),"=d"(d):"a"(0u),"c"(0u));uint32_t max_leaf=a;
