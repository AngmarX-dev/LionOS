...
int paging_map_user_page_in(uint32_t pd_physical, uint32_t virtual_address,
    ...
    uint32_t *table;
    if ((pde & 0x1u) && (pde & 0x4u)) table = directory_ptr(pde & 0xFFFFF000u);
    else {
    ...
    if (current_directory == pd_physical) __asm__ volatile ("invlpg (%0)" : : "r"(virtual_address) : "memory");
    return 0;
}
...
void paging_destroy_address_space(uint32_t pd_physical) {
    ...
    for (uint32_t i = 0; i < PAGE_ENTRIES; ++i) {
        ...
        if ((pde & 0x5u) == 0x5u) page_free((void *)(uintptr_t)(pde & 0xFFFFF000u));
    }
    page_free(directory);
}
